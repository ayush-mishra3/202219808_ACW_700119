...
TSCoralRenderer::TSCoralRenderer(const std::shared_ptr<DX::DeviceResources>& deviceResources) :
...
void TSCoralRenderer::CreateWindowSizeDependentResources()
{
...
void TSCoralRenderer::CreateDeviceDependentResources()
{
	// Load shaders asynchronously.
	auto loadVSTask = DX::ReadDataAsync(L"VertexShader06.cso");
	auto loadHSTask = DX::ReadDataAsync(L"HullShader06.cso");
	auto loadDSTask = DX::ReadDataAsync(L"DomainShader06.cso");
	auto loadPSTask = DX::ReadDataAsync(L"PixelShader06.cso");
	...
	CD3D11_BUFFER_DESC tessBufferDesc(sizeof(TessBuffer), D3D11_BIND_CONSTANT_BUFFER);
	DX::ThrowIfFailed(
		m_deviceResources->GetD3DDevice()->CreateBuffer(
			&tessBufferDesc,
			nullptr,
...
void TSCoralRenderer::ReleaseDeviceDependentResources()
{
	m_loadingComplete = false;
...
void TSCoralRenderer::Update(DX::StepTimer const& timer)
{
	m_tessBufferData.tessAmount = m_tessFactor;
...
void TSCoralRenderer::Render()
{
	...
	m_tessBufferData.tessAmount = 31.0;
